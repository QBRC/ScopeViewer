...
def ensure_format(genelist, counts, locations):
    if(len(genelist) == len(counts) and len(counts[0]) == len(locations)):
        return True
    else:
        return False
...
def main():
    ...
    gene_list_response = urlopen("http://cbio.biohpc.swmed.edu/imgviewer_ffpe_bc/smp_data/genelist_remoteurl.json")
    gene_list = json.loads(gene_list_response.read())
    # replace with a path to smp count csv file
    smp_count_file =  open('human_breast_cancer_ffpe.count.csv')
    smp_count_reader = csv.reader(smp_count_file)
    count_list = list(smp_count_reader)
    # replace with a path to location csv file
    smp_loc_file = open('human_breast_cancer_ffpe.loc.csv')
